...
fn sync_gap_test() {
    for gap in [1, 2, 3].iter() {
        ...
        assert_eq!(status.current_height, max_height);
    }
}
...
impl CommonConsensusAdapter for MockCommonConsensusAdapter {
    ...
    async fn verify_txs(&self, _: Context, _: Vec<Hash>) -> ProtocolResult<()> {
        // assuming mempool works fine and have all needed txs which has been validated
        Ok(())
    }
    ...
    async fn verify_block_header(&self, ctx: Context, block: &Block) -> ProtocolResult<()> {
        ...
        if previous_block_hash != block.header.pre_hash {
            return Err(ConsensusError::SyncBlockHashErr(block.header.height).into());
        }
        ...
        if block.header.pre_hash != block.header.proof.block_hash && block.header.height > 1 {
            ...
            return Err(ConsensusError::SyncBlockPreHashErr(block.header.height).into());
        }

        // todo verify proposer and validators
    ...
    async fn verify_proof(
        ...
        // the block 0 has no proof, hich is consensus-ed by community, not by chain
        ...
        match self.crypto.verify_aggregated_signature(
            ...
            Err(_) => {
                return Err(
                    ConsensusError::ConsensusVerifyBlockProofErr(block.header.height).into(),
                );
            }
        };
        ...
        for signed_voter_address in signed_voters {
            if weight_map.contains_key(signed_voter_address.as_ref()) {
                ...
                return Err(
                    ConsensusError::ConsensusVerifyBlockVoterErr(block.header.height).into(),
                );
            }
        }
        ...
        if 3 * accumulator <= 2 * total_validator_weight {
            return Err(
                ConsensusError::ConsensusVerifyBlockVoteWeightErr(block.header.height).into(),
            );
        }
...
// 通过SingedTx获得对应的TxReceipt, 对应Tx的state_root为empty
...
fn assert_sync(status: CurrentConsensusStatus, latest_block: Block) {
    ...
    assert_eq!(status.current_proof.height, status.current_height);
