...
pub enum ContractType {
    // Asset contract
    #[graphql(description = "Asset contract often use for creating User Define Asset(also known as (User Define Token))")]
    Asset,
    // App contract, the code in the contract is allowed to change the state world.
    #[graphql(description = "App contract often use for creating DAPP(Decentralized APPlication) ")]
    App,
...
pub struct InputRawTransaction {
    ...
    pub chain_id: Hash,
    #[graphql(description = "Mostly like the gas limit in Ethereum, describes the fee that \
