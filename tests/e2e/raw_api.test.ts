...
describe("query API works", () => {
  test("getBlock works", async () => {
    ...
    expect(typeof res.data.getBlock.header.height).toBe("string");
  });

  test("getBlock with height works", async () => {
    let q = `
        query {
            getBlock(height: "0x0") {
                header {
        ...
        `;
    let res = await client.query({ query: gql(q) });
    expect(res.data.getBlock.header.height).toBe("0x0000000000000000");
  });
});
...
mutation {
  sendUnsafeTransferTransaction(
    ...
    inputAction: {
      ...
      receiver: "0x100000000000000000000000000000000000000000"
    },
