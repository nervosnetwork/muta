...
use crate::{
    ...
    proto::common::{Empty, Result as SrvResult},
...
impl<T: SyncService + Sync + Send + 'static> GrpcSyncService for GrpcSyncImpl<T> {
    fn update_status(&self, o: RequestOptions, status: Status) -> SingleResponse<SrvResult> {
        self.core_srv
    ...
    fn proc_sync_response(&self, o: RequestOptions, resp: SyncResp) -> SingleResponse<SrvResult> {
        self.core_srv
            .proc_sync_response(Context::from_rpc_context(o), resp)
            .into_single_resp()
    }

    fn get_peer_count(&self, o: RequestOptions, _: Empty) -> SingleResponse<PeerCountResp> {
        self.core_srv
